...
public class EquipoService {
    ...
    public List<EquipoDTO> buscarEquiposComplejos(String marca) {
        // Usamos una consulta tipo HQL para filtrar con más precisión
        List<Equipo> resultados = Equipo.list("marca = ?1 and categoria is not null", marca);

        // Lo convertimos a DTO para mantener la buena práctica
        return resultados.stream().map(e -> {
            EquipoDTO dto = new EquipoDTO();
            dto.id = e.id;
            dto.marca = e.marca;
            dto.modelo = e.modelo;
            dto.nombreCategoria = e.categoria.nombre;
            return dto;
        }).toList();
    }
}